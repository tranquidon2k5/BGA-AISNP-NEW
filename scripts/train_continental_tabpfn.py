"""
Train TabPFN cho phân loại châu lục (Continental tier).
Cùng pattern với train_continental_xgb.py.
"""
import os
import sys
...
def main():
    ...
    # TabPFN không cần num_classes vì tự detect từ y_train
    ...
    acc, _ = train_and_eval(
        ...
        y_test,
        label_encoder=le,
        title="TabPFN Continental Ancestry",
    )
    ...
    joblib.dump(model_full, os.path.join("models", "continent_tabpfn.pkl"))
    joblib.dump(le, os.path.join("models", "continent_tabpfn_label_encoder.pkl"))
    joblib.dump(snp_names, os.path.join("models", "continent_tabpfn_snp_names.pkl"))

    print(f"Saved continental TabPFN model (acc on test={acc:.4f}) to models/.")
